...
class PPasswordViewController: UIViewController {
	...
	override func viewDidLoad() {
		...
		passwordView = PasswordView(frame: CGRectMake(0, 64, self.view.frame.size.width, 61.0))
		self.view.addSubview(passwordView!)

		var progressValue:Float = 4.0 / maxChars

		passwordLength = UIProgressView(progressViewStyle: UIProgressViewStyle.Default)
		passwordLength?.frame = CGRectMake(0, passwordView!.frame.origin.y + PasswordView.height(), self.view.frame.size.width, 1.0);
		...
		passwordLength?.trackTintColor = UIColor.passwordTextColor()
		self.view.addSubview(passwordLength!)

		let totalHeight:CGFloat = self.view.frame.size.height - 64.0 + PasswordView.height() + 2.0
		...
		view1.backgroundColor = UIColor.passwordItemColor()
		...
		view3.backgroundColor = UIColor.passwordItemColor()
		...
		view5.backgroundColor = UIColor.passwordItemColor()
		...
		lengthSlider?.tintColor = UIColor.passwordTextColor()
		...
		lengthValueLabel?.textColor = UIColor.passwordTextColor()
		...
		let attributes:NSDictionary = NSDictionary(objectsAndKeys:
			...
			UIColor.passwordMainColor(), NSForegroundColorAttributeName)

		let lettersArr:Array = ["abc", "aBc", "ABC"]

		letters = UISegmentedControl(items: lettersArr)
		letters?.tintColor = UIColor.passwordMainColor()
		...
		typeChar?.tintColor = UIColor.passwordMainColor()
		...
		digitsLabel?.textColor = UIColor.passwordTextColor()
		...
		digitsSlider?.tintColor = UIColor.passwordTextColor()
		...
		digitsValueLabel?.textColor = UIColor.passwordTextColor()
		...
		symbolLabel?.textColor = UIColor.passwordTextColor()
		...
		symbolSlider?.tintColor = UIColor.passwordTextColor()
		...
		symbolValueLabel?.textColor = UIColor.passwordTextColor()
		...
		avoidAmbiguousLabel?.textColor = UIColor.passwordTextColor()
		...
		switchAmbiguous?.onTintColor = UIColor.passwordMainColor()
		...
		allowCharsRepeatLabel?.textColor = UIColor.passwordTextColor()
		...
		switchAllowRepeat?.onTintColor = UIColor.passwordMainColor()
		...
		secureLabel?.textColor = UIColor.passwordTextColor()
		...
		passwordMode?.onTintColor = UIColor.passwordMainColor()
		...
		clipboardLabel?.textColor = UIColor.passwordTextColor()
		...
		clipboardSwitch?.onTintColor = UIColor.passwordMainColor()
		...
		titleMainView.text = "Password Generator"
	...
	func displaySettings(sender: UIButton) {
		let settingsViewController:SettingsTableViewController = SettingsTableViewController(style: UITableViewStyle.Grouped)
	...
	func generatePassword() {
		...
		let pc:PasswordController = PasswordController.sharedInstance
		var password:String
	...
	func displayText(value:NSString) {
		...
		for match in matches {
			attributedString.addAttribute(NSForegroundColorAttributeName, value: UIColor.passwordNumbersColor(), range: match.range)
		...
		for match in matchesLetters {
			attributedString.addAttribute(NSForegroundColorAttributeName, value: UIColor.passwordMainColor(), range: match.range)
		}
		...
		for match in matchesLetters {
		mutableAttributedString.addAttribute(NSForegroundColorAttributeName, value: UIColor.passwordMainColor(), range: match.range)
		}
